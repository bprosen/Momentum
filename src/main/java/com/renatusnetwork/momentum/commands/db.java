            ...
            new BukkitRunnable()
            {
                ...
                public void run()
                {
                    try
                    {
                        ...
                        try
                        {
                            File oldLevels = new File(Momentum.getPlugin().getDataFolder(), "levels.yml");
                            levelsConfig = new YamlConfiguration();
                            levelsConfig.load(oldLevels);

                            File oldPerks = new File(Momentum.getPlugin().getDataFolder(), "perks.yml");
                            perksConfig = new YamlConfiguration();
                            perksConfig.load(oldPerks);

                            File locationPerks = new File(Momentum.getPlugin().getDataFolder(), "locations.yml");
                        ...
                        if (!cannotContinue)
                        {
                            ...
                            Momentum.getPluginLogger().info("Opening db connection to second db");
                            open();
                             ...
                             */
                            Momentum.getPluginLogger().info("Attempting migration of players table");
                            ...
                            while (results.next())
                            {
                                ...
                                if (infiniteBlock.isEmpty())
                                    infiniteBlock = Momentum.getSettingsManager().infinite_default_block.name();
                             ...
                             */
                            Momentum.getPluginLogger().info("Attempting migration of levels.yml to levels table");
                            ...
                            while (results.next())
                            {
                                ...
                                if (oldDate != date)
                                {
                                    Momentum.getPluginLogger().info("Duplicate completion from forced found for " + uuid + ", changing date from " + oldDate + " -> " + date);
                                    duplicates++;
                                }
                             ...
                             */
                            Momentum.getPluginLogger().info("Attempting migration of perks table");
                            ...
                            /*
                                Perks migration system
                             */
                            Momentum.getPluginLogger().info("Attempting migration of locations");
                            ...
                            Momentum.getPluginLogger().info("Enabling foreign key checks");
                            DatabaseQueries.runQuery("SET foreign_key_checks = 1");
                            close();
                            Momentum.getPluginLogger().info("Closing database connection");
                        }
                    }
