...
public class ClanCMD implements CommandExecutor
{
        ...
        if (a.length > 0)
        {
            ...
            else if (a.length == 2 && a[0].equalsIgnoreCase("addmaxlevel"))
            {
                if (sender.hasPermission("momentum.admin"))
                {
                    ...
                    if (target != null)
                    {
                        Clan targetClan = Momentum.getStatsManager().get(target).getClan();
            ...
            else if (sender instanceof Player)
            {
                ...
                else if (a.length == 2 && a[0].equalsIgnoreCase("create"))
                {
                    ...
                    if (clan == null)
                    {
                        ...
                        int createPrice = Momentum.getSettingsManager().clans_price_create;
                    ...
                    if (clan != null)
                    {
                        if (clan.isOwner(player.getName()))
                        {
                            if (a.length > 1)
                            {
                                ...
                                int price = Momentum.getSettingsManager().clans_price_tag;
    ...
    private static boolean clanTagRequirements(String clanTag, CommandSender sender)
    {
            ...
            clanTag.length() > Momentum.getSettingsManager().clans_tag_length_max)
        {
            ...
            sender.sendMessage(Utils.translate(
                    "&cClan Tags must be &4" + Momentum.getSettingsManager().clans_tag_length_min + "-" +
                         Momentum.getSettingsManager().clans_tag_length_max + " &ccharacters"
            ));
            return false;
        }
        else if (Momentum.getClansManager().existsIgnoreCase(clanTag))
        {
    ...
    private static void sendHelp(CommandSender sender)
    {
        ...
        sender.sendMessage(Utils.translate("&3/clan create <tag>  &7Create a clan &6" + Momentum.getSettingsManager().clans_price_create + " Coins"));
        sender.sendMessage(Utils.translate("&3/clan changetag <tag>  &7Change clan tag &6" + Momentum.getSettingsManager().clans_price_tag + " Coins"));
        ...
        else if (sender.hasPermission("momentum.staff"))
