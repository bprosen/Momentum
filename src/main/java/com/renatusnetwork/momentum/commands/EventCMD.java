            ...
            // start event
                ...
                if (!eventManager.isEventRunning()) {
                    try {
                        ...
                        switch (eventType) {
                            case PVP:
                                if (Momentum.getLevelManager().getPvPEventLevels().isEmpty()) {
                                    player.sendMessage(Utils.translate("&cThere are no levels for &4PvP Event &cavailable"));
                                    return true;
                                }
                                break;
                            case FALLING_ANVIL:
                                if (Momentum.getLevelManager().getFallingAnvilEventLevels().isEmpty()) {
                                    player.sendMessage(Utils.translate("&cThere are no levels for &4Falling Anvil Event &cavailable"));
                                    return true;
                                }
                                break;
                            case RISING_WATER:
                                if (Momentum.getLevelManager().getRisingWaterEventLevels().isEmpty()) {
