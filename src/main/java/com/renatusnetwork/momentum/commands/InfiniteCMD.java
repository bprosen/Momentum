...
public class InfiniteCMD implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] a) {
        ...
        // admin command for removing leaderboard position
        } else if (player.hasPermission("momentum.admin") && (a.length == 3 && a[0].equalsIgnoreCase("setscore"))) {
            if (Utils.isInteger(a[2])) {
                if (InfinitePKDB.hasScore(a[1])) {
                    ...
                    }.runTaskAsynchronously(Momentum.getPlugin()); // load lb
        ...
        } else if (player.hasPermission("momentum.admin") && (a.length == 1 && a[0].equalsIgnoreCase("setportalrespawn"))) {
            ...
            // set and save
            ConfigManager configManager = Momentum.getConfigManager();
            configManager.get("settings").set("infinitepk.portal_respawn", locString);
            configManager.save("settings");
            Momentum.getSettingsManager().load(configManager.get("settings"));
            ...
            Momentum.getInfinitePKManager().clearRewards();
            ...
            LinkedHashMap<Integer, InfinitePKReward> rewards = Momentum.getInfinitePKManager().getRewards();
