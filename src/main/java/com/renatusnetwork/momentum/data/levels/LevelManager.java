...
public class LevelManager {
    ...
    private HashMap<Integer, Level> globalLevelCompletionsLB = new HashMap<>
            (Momentum.getSettingsManager().max_global_level_completions_leaderboard_size);
    private HashMap<Integer, Level> topRatedLevelsLB = new HashMap<>
            (Momentum.getSettingsManager().max_rated_levels_leaderboard_size);
    ...
    public LevelManager(Plugin plugin) {
        ...
        tutorialLevel = get(Momentum.getSettingsManager().tutorialLevelName);
    ...
    private void startScheduler(Plugin plugin) {
        ...
        }.runTaskTimerAsynchronously(Momentum.getPlugin(), 20 * 15, 20 * 180);
    }
    ...
    public void remove(String levelName) {
        ...
        if (foundLevel) {
            ...
            Momentum.getDatabaseManager().runAsyncQuery("DELETE FROM levels WHERE level_name='" + levelName + "'");
            Momentum.getDatabaseManager().runAsyncQuery("DELETE FROM checkpoints WHERE level_name='" + levelName + "'");
            Momentum.getDatabaseManager().runAsyncQuery("DELETE FROM completions WHERE level_id=" + level.getID());
