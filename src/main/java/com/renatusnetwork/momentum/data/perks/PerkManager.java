...
public class PerkManager {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (PerksDB.syncPerkIDs()) {
                    Momentum.getPerkManager().setIDCache(PerksDB.getIDCache());
                    PerksDB.syncIDCache();
                }
            }
    ...
    public void setPerk(Perk perk, PlayerStats playerStats)
    {
        ...
        else if (perk.isInfinitePKBlock())
        {
            ...
            Momentum.getDatabaseManager().runAsyncQuery("UPDATE players SET infinite_block='" + perk.getInfinitePKBlock().name() + "' WHERE uuid='" + playerStats.getUUID() + "'");
        }
    }
}