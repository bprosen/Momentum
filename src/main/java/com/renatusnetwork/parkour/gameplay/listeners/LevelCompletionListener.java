...
public class LevelCompletionListener implements Listener
{

    @EventHandler
    public void onCompletion(LevelCompletionEvent event)
    {
        PlayerStats playerStats = event.getPlayerStats();

        if (playerStats != null)
        {
            Modifier modifier = playerStats.getModifier(ModifierTypes.LEVEL_BOOSTER);
